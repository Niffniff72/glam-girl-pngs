...
1. **Payhip links.** Every `href="#payhip-..."` in `index.html` and the footer is a placeholder. Replace with your real Payhip storefront and category URLs (`id="payhip-tumblers"`, `#payhip-tees`, `#payhip-journals`, `#payhip-shop`).
...
4. **Pay in 4.** The payment section is written around **PayPal Pay Later**, which shows up automatically at PayPal checkout for eligible U.S. orders — sellers don't need to build anything extra for it. Before you launch, confirm in your Payhip account that PayPal is connected as a payment method and that Pay Later is showing for your order sizes (PayPal decides eligibility per order, not per seller). If you'd rather offer Klarna or Afterpay instead/also, that depends on which processor Payhip has enabled in your account at the time — check Payhip's current payment settings, since this changes as they roll features out.
